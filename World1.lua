...
-- Universal Peak-Rate Auto Rebirth Loop (Ported from World 2)
task.spawn(function()
    while _G_CONFIG.ScriptRunning do
...
AutomationTab:Toggle({
    Title = "Auto Rebirth",
    Desc = "Automatically rebirths using peak rate logic.",
...
AutomationTab:Toggle({
    ...
    Desc = "Automatically ascend whenever available.",
...
AutomationTab:Button({
    Title = "Ascend Now",
    Desc = "Invoke ascension remote immediately.",
    Callback = function()
        forceManualAscend()
    end
})
...
WorldTab:Toggle({
    Title = "Magnet Cash Bags",
    Desc = "Attracts spawned cash drops towards player. [Locked]",
    Value = _G_CONFIG.AutoMagnet,
    Locked = true,
    LockedTitle = "Under Maintenance",
    LockReason = "Under Maintenance",
    Callback = function(Value)
        notify("Locked", "Magnet Cash Bags is locked by developer.", 3, "alert")
        _G_CONFIG.AutoMagnet = false
    end
})
...
WorldTab:Button({
    ...
    Desc = "Teleports to World 2. (Must be Unlocked)",
    Callback = function()
        notify("Teleport", "Teleporting to World 2...", 3, "globe")
        TeleportService:Teleport(WORLD_2_PLACE_ID, LocalPlayer)
    end
})
...
task.spawn(function()
    ...
    notify("kraken Loaded", "Script loaded successfully. World 1 detected.", 5, "check")
end)
